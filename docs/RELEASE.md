...
Current planned release: `v0.1.5`

Expected macOS release assets:

- `HNS-Investments-0.1.5-arm64.dmg`
- `HNS-Investments-0.1.5-x64.dmg`
- `SHA256SUMS.txt`
...
codesign --verify --verbose=2 "dist/HNS-Investments-0.1.5-arm64.dmg"
xcrun stapler validate "dist/HNS-Investments-0.1.5-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose "dist/HNS-Investments-0.1.5-arm64.dmg"
hdiutil verify "dist/HNS-Investments-0.1.5-arm64.dmg"
(cd dist && shasum -a 256 HNS-Investments-0.1.5-*.dmg > SHA256SUMS.txt)
...
gh release create v0.1.5 \
  dist/HNS-Investments-0.1.5-arm64.dmg \
  dist/HNS-Investments-0.1.5-x64.dmg \
  dist/SHA256SUMS.txt \
  --repo shadstoneofficial/hns-investments-companion \
  --title "HNS Investments v0.1.5" \
  --prerelease \
  --notes-file docs/RELEASE_NOTES_v0.1.5.md
```

Verify:

```bash
gh release view v0.1.5 --repo shadstoneofficial/hns-investments-companion --json tagName,targetCommitish,isPrerelease,url,assets
```
