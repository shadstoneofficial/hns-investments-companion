...
Current planned release: `v0.1.2`
...
codesign --verify --verbose=2 "dist/HNS-Investments-0.1.2-arm64.dmg"
xcrun stapler validate "dist/HNS-Investments-0.1.2-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose "dist/HNS-Investments-0.1.2-arm64.dmg"
hdiutil verify "dist/HNS-Investments-0.1.2-arm64.dmg"
(cd dist && shasum -a 256 HNS-Investments-0.1.2-*.dmg > SHA256SUMS.txt)
...
gh release create v0.1.2 \
  dist/HNS-Investments-0.1.2-arm64.dmg \
  dist/HNS-Investments-0.1.2-x64.dmg \
  dist/SHA256SUMS.txt \
  --repo shadstoneofficial/hns-investments-companion \
  --title "HNS Investments v0.1.2" \
  --prerelease \
  --notes-file docs/RELEASE_NOTES_v0.1.2.md
```

Verify:

```bash
gh release view v0.1.2 --repo shadstoneofficial/hns-investments-companion --json tagName,targetCommitish,isPrerelease,url,assets
```
